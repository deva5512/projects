...
public class CandidateView implements Teleportable {
	...
	public void caller(Object obj) {
		...
		while (true) {
			...
			if (input == 1) {
				...
				if (candidateModel.getResumeStatus()) {
					viewJobs();
				} else {
					System.out.println("Kindly Update Your resume before looking for a job ");
				}
			} else if (input == 3) {
				resume();
			} else if (input == 4) {
	...
	private void modifyCandidateDetails() {
		...
		if (input == 1) {
			...
			candidateModel.changeUserDetails(name, null);
			...
			candidateModel.changeUserDetails(null, pass);
	...
	private void resume() {
		...
		candidateModel.createResume(name, qualification, YOP, skill, number, mail);
	...
	public void viewJobs() {
		...
		List<Post> list = candidateModel.getPost();
