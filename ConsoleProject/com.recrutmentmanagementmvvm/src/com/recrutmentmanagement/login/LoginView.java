...
public class LoginView {
	...
	private void userLogin() {
		...
		if (hr) {
			Hr hrdetails = loginModel.hrLoginCredential(name, pass);

			if (hrdetails != null) {
				hrLogin(hrdetails);
			} else {
				verificationFailed("Invalid user name or password ");
			}

		} else {

			Candidate candidate = loginModel.userLoignCredential(name, pass);
	...
	private void newUserRegistration() {
		...
		if (hr) {
			...
			Hr hrdetails = loginModel.registerHr(name, pass, organization);
			if (hrdetails != null) {
				hrLogin(hrdetails);
			} else {
				verificationFailed("Invalid user name or password ");
			...
			Candidate candidate = loginModel.registerCandidate(name, pass);
	...
	private void adminLogin() {
		...
		if (loginModel.checkForAdmin(name, pass)) {
			adminVerified(name);
		} else {
			verificationFailed("Improper User name or Password");
		}
	}
	...
	private void adminVerified(String name) {
		System.out.println("\n -> Welcome " + name + " <-");

		Queue<String> notification = loginModel.getNotification();
	...
	private void viewPaymentHistory() {
		List<Post> post = loginModel.getPostHistory();
	...
	private void mofifyAdminInfo() {
		...
		if (input == 1) {
			...
			loginModel.changeAdminDetails(name, null, 0);
			...
			loginModel.changeAdminDetails(null, pass, 0);
			...
			loginModel.changeAdminDetails(null, null, phno);
	...
	private void viewAdminInfo() {
		Admin admin = loginModel.getAdmin();
	...
	private void candidateLogin(Candidate candidate) {
